...
func TestSmallPartitionConsensus2A(t *testing.T) {
	fmt.Printf("==================== 3 SERVERS ====================\n")
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()

	fmt.Printf("Test (2A): SmallPartition Consensus\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()

	// if the leader disconnects, a new one should be elected.
	fmt.Printf("Forming partition2 with leader and 1 nodes\n")
	...
	for k := 0; k < cfg.n; k++ {
		if k != leader1 {
			partition2[k] = struct{}{}
			fmt.Println("Leader1", leader1, "node:", k)
			break
		}
	}
	for k := 0; k < cfg.n; k++ {
		if _, exists := partition2[k]; exists == false{
			fmt.Println("Partition1:", k)
			partition1[k] = struct{}{}
		}
	}
	...
	// a new leader should be elected
	fmt.Println("Checking for a new leader in original partition")
	cfg.checkLeaderInPartition(partition1)

	fmt.Printf("Checking for previous leader in partition2\n")
	oldLeader := cfg.checkLeaderInPartition(partition2)
	fmt.Println("Leader found in partition2:", oldLeader, "original:", leader1)
	if oldLeader != leader1 {
		cfg.t.Fatalf("Found a different leader in partition")
	}

	fmt.Printf("======================= END =======================\n\n")
}

func TestSmallPartitionNoConsensus2A(t *testing.T) {
	fmt.Printf("==================== 5 SERVERS ====================\n")
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()

	fmt.Printf("Test (2A): SmallPartition NoConsensus\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()

	// if the leader disconnects, a new one should be elected.
	fmt.Printf("Forming partition2 with leader and 2 nodes\n")
	var partitionOrig IntSet
	var partition2 IntSet
	partitionOrig = make(map[int]struct{})
	partition2 = make(map[int]struct{})
	partition2[leader1] = struct{}{}
	fmt.Println("partition2 leader:", leader1)
	count := 1
	for k := 0; k < cfg.n; k++ {
		if k != leader1 {
			partition2[k] = struct{}{}
			fmt.Println("partition2 node:", k)
			count += 1
		}
		if count == 3 {
			break
	...
	for k := 0; k < cfg.n; k++ {
		if _, exists := partition2[k]; exists == false{
			fmt.Println("Partition-orig:", k)
			partitionOrig[k] = struct{}{}
		}
	}
	cfg.disconnect_partition(partition2)
	// fmt.Printf("Disconnecting leader\n")
	...
	// a new leader should be elected
	fmt.Println("Checking that theres no new leader in original partition")
	cfg.checkNoLeaderInPartition(partitionOrig)
...
func TestLargePartitionConsensus2A(t *testing.T) {
	...
	fmt.Printf("Test (2A): LargePartition Consensus\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()

	// if the leader disconnects, a new one should be elected.
	fmt.Printf("Forming partition2 with leader and 3 nodes\n")
	var partitionOrig IntSet
	var partition2 IntSet
	partitionOrig = make(map[int]struct{})
	partition2 = make(map[int]struct{})
	...
	for k := 0; k < cfg.n; k++ {
		...
		if partition2Count == 4 {
			break
		}
	}
	for k := 0; k < cfg.n; k++ {
		if _, exists := partition2[k]; exists == false{
			fmt.Println("PartitionOrig:", k)
			partitionOrig[k] = struct{}{}
		}
	}
	fmt.Println("PartitionOrig: ", partitionOrig)
	cfg.disconnect_partition(partition2)

	// a new leader should be elected
	fmt.Println("Checking for a new leader in original partition")
	cfg.checkLeaderInPartition(partitionOrig)
	...
	fmt.Println("Reconnecting Partition2")
	cfg.connect_partition(partition2) // Reconnecting partition2
	// Waiting a bit
...
func TestLargePartitionNoConsensus2A(t *testing.T) {
	fmt.Printf("==================== 11 SERVERS ====================\n")
	servers := 11
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()

	fmt.Printf("Test (2A): LargePartition No Consensus\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()
	...
	// a new leader should be elected
	fmt.Println("Checking for a new leader in original partition: should be no leader")
	cfg.checkNoLeaderInPartition(partitionOrig)
...
func TestFailNoAgree2C(t *testing.T) {
	fmt.Printf("==================== 5 SERVERS ====================\n")
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
...
func (cfg *config) checkLeaderInPartition(partition IntSet) int {
	fmt.Println("HERERERE", len(partition))
	for iters := 0; iters < 10; iters++ {
		time.Sleep(500 * time.Millisecond)
		leaders := make(map[int][]int)
		for i, _ := range partition {
			// Don't check if connected, since state may be unclear
			_, t, leader := cfg.rafts[i].GetState()
			fmt.Println("Checking ", i, "term", t, "is leader", leader)
			if _, t, leader := cfg.rafts[i].GetState(); leader {
				leaders[t] = append(leaders[t], i)
			}
		}
...
func (cfg *config) one(cmd int, expectedServers int) int {
	...
	for time.Since(t0).Seconds() < 10 {
		...
		fmt.Println("FOund leader", index)
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				cmd2, ok := cmd1.(int)
				fmt.Println("One: looking for commited cmd", cmd, ok, cmd2, "num commited", nd)
